...
void InitializeData()
{
	...
	//新增5筆學生資料
	students.push_back(Student("I001", "陳", "小明", "男", "1999-01-01", "S001", Depertment::ComputerSciece, ClassName::_1A));
	students.push_back(Student("I002", "林", "小華", "男", "1999-12-31", "S002", Depertment::ElectricalEngineering, ClassName::_1A));
	students.push_back(Student("I003", "黃", "小美", "女", "1999-06-01", "S003", Depertment::IformationManagement, ClassName::_1B));
	students.push_back(Student("I004", "張", "小強", "男", "1999-07-31", "S004", Depertment::ComputerSciece, ClassName::_2A));
	students.push_back(Student("I005", "李", "小花", "女", "1999-08-01", "S005", Depertment::ElectronicEngineering, ClassName::_2B));

	//新增2筆老師資料
	vector<Course> teacher1_courses = { courses[0], courses[1], courses[2] };
	teachers.push_back(Teacher("I101", "王", "大富", "男", "1980-07-01", "T001", Depertment::ComputerSciece, ClassName::_1A, teacher1_courses));
	vector<Course> teacher2_courses = { courses[3], courses[4] };
	teachers.push_back(Teacher("I102", "李", "大貴", "男", "1985-05-05", "T002", Depertment::IformationManagement, ClassName::_1B, teacher2_courses));
...
void addStudent()
{
	string id, lastName, firstName, birthData, gender, studentId;
	...
	{
		cout << i << ". " << Utility1::toString(static_cast<Depertment>(i)) << endl;
	}
	cout << "請選擇科系: ";
	cin >> departmentChoice;
	Depertment department = static_cast<Depertment>(departmentChoice);

	cout << "班級: " << endl;
	for (int i = 0; i < static_cast <int>(ClassName::Last); i++)
	{
		cout << i << ". " << Utility1::toString(static_cast<ClassName>(i)) << endl;
	}
	cout << "請選擇班級: ";
	cin >> classNameChoice;
	ClassName className = static_cast<ClassName>(classNameChoice);

	students.push_back(Student(id, lastName, firstName, gender, birthData, studentId, department, className));
}
...
void saveRecordsToFile(const string& filename) {
	...
	for (const auto& record : records) {
		...
		if (studentIt != students.end() && courseIt != courses.end()) {
			...
			outFile << "生日: " << studentIt->getBirthData() << endl;
			outFile << "科系: " << Utility1::toString(studentIt->getDepartment()) << endl;
			outFile << "班級: " << Utility1::toString(studentIt->getClassName()) << endl;
			outFile << "----------------" << endl;
			outFile << "課程資料:" << endl;
			outFile << "----------------" << endl;
